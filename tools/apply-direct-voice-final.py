...
REPLACEMENTS = {
    "scripts/03-2_give-each-dollar-a-job-before-choosing-the-investment.md": [
        (
            ...
            "Those ranges help explain the pattern.\n\nThe current app makes the retirement access question more specific: Reserve is available now, Bridge is the money needed before age fifty-nine and a half, and Legacy is the money for after that point.",
        )
    ],
    ...
    "scripts/08-1_choose-who-is-in-charge-and-put-the-legal-baseline-in-place.md": [
        (
            ...
            "A power of attorney generally ends at death. The executor or trustee role takes over then.\n\nA healthcare directive covers medical decisions and wishes.",
        )
    ],
...
def flexible_pattern(old: str) -> re.Pattern[str]:
    # Audit findings collapse paragraph whitespace. Match the same prose whether
    # the two sentences share a paragraph or have one blank line between them.
    chunks = re.split(r"\s+", old.strip())
    return re.compile(r"\s+".join(re.escape(chunk) for chunk in chunks))
