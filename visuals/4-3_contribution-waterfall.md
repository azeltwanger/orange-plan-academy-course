# 4.3 · Three gates, then the fork

**Paste `00-STYLE.md` first, then this.**

## What it has to make obvious
That routing is not one long ladder. Three conditional gates settle first, and then the decision **forks on time horizon** — because when you need the money, not which account is prettiest, is what picks the destination.

## The visual
Left third: three gates stacked vertically, each drawn as a checkpoint that can be **passed through or stamped "not applicable"** — deliberately not as rungs of a waterfall. Reserve, employer match, extra debt.
