# Session 2 — Build cash flow, the Reserve, and expected life events

Playback alternates the concept lessons with matching W02 chapters. College is optional. Session 3 decides extra debt; Session 4 finalizes investment routing. This session identifies the available pool and the reserve claim without spending the same surplus twice.
...
In the Reed example, the existing education account already has a specific job. We keep it separate from the general retirement portfolio. We then compare the family commitment with that account and the other sources they are willing to use. If there is a gap, we can change the saving pace, the commitment, the timing of other spending, or the funding mix.

The app may model education events and dedicated accounts, but the family still chooses the commitment. In the working-session chapter, use only the education funding and savings calculations the current build actually supports. A separate worksheet can make the family decision explicit without claiming the software has optimized every college choice.
