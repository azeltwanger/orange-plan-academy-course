...
So the same concentration question from the last section applies here too. If every satoshi you own is sitting behind one model of one device, running one company's firmware, generated by one implementation, then you've got a single trust holding up your entire stack. That's the same shape as keeping everything at one exchange. It's just harder to see, because it feels like independence.
