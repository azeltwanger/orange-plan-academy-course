# Unit 5 · Module 4 — Allocation & Next-Dollar

*Pick an allocation tier, stress-test what you can actually hold, split your money into Reserve / Bridge / Legacy, order your contributions, and put each holding in the right account type.*

## 4.1 Set the Bitcoin allocation you can hold through a full drawdown
*`TEACH` · 1,464 words · ~9 min*

**By the end of this lesson, you can:**

- Measure your Bitcoin allocation against the right denominator
- Place yourself in a tier using education, conviction, and comfort with volatility
- Price a 70 to 80% drawdown in dollars, not percentages
- Set the largest position you could hold through one without selling
- Check that your custody level supports the position

---

Your allocation is the share of your money in Bitcoin. That percentage changes depending on what you divide by, so the same household can look aggressive or conservative without owning a single different coin.

### What you divide by

Take the couple. They hold 1.75 Bitcoin at $100,000/coin, or $175,000. Their non-Bitcoin: $90,000 index funds, $30,000 cash, $450,000 home equity.

- **Investable only:** $175k ÷ $295k = about **60%**.
- **Total net worth:** $175k ÷ $745k = about **24%**.

Both are legitimate. Investable shows how the growth engine is running. Net worth shows how exposed your balance sheet is.

### What sets a responsible allocation

What actually sets it is three things:

1. **Education.** How much you know about what Bitcoin is and why it works.
2. **Conviction.** What that knowledge turns into once you've studied enough to defend the position.
3. **Comfort with volatility.** What you'd actually do in a 70% drawdown, not what you'd like to think you'd do.

Together they set the size of the position you can survive. That's different from the position you might want.

Austin's own: ~37% average annual Bitcoin returns over the last five years, and he's watched his net worth drop 75% in a single drawdown and held. Because he understood what he owned.

### The four tiers

**Foundation: 1 to 5%**

New and curious. Bitcoin is a small hedge.

- **Custody.** Exchange or ETF.
- **The job.** Learning why Bitcoin exists. Read the whitepaper. Understand what makes Bitcoin different.

**Integration: 10 to 20%**

You believe Bitcoin is a real part of the plan.

- **Custody.** Start learning self-custody. Buy a hardware wallet, send a test transaction, move a small amount off exchange first.
- **The job.** Get comfortable with the mechanics.
- **Common mistake.** Integration-level allocation with Foundation-level custody. Coins on an exchange while claiming to be 15% in.

**Optimization: 25 to 50%**

You believe the thesis and have held through volatility. Bitcoin is a main growth driver.

- **Custody.** Confident self-custody. Coins on hardware wallets. Recovery test done (Module 7). Passphrase understood — the optional extra word layered on top of your seed words — even if not used yet.
- **The job.** Operational discipline. Beneficiaries, heir letter, dead man's switch (Module 8).
- **The stakes.** One custody mistake can meaningfully damage the plan.

**Sovereign: 50%+**

Bitcoin as future money. Holding anything else feels like the risk.

- **Custody.** Optimized. Could be single-sig, multisig (more than one key required to spend), collaborative, or a mix.
- **The job.** The risks are operational, legal, and political now. "Should I buy Bitcoin?" is done. "How do I hold and pass it on?" is the question.

### Pair allocation with custody

Two numbers describe your Bitcoin situation:

- Your **allocation level** (%)
- Your **custody level** (exchange, ETF, hardware wallet, multisig)

When both are at the same tier, the position is supported. When allocation is a tier ahead of custody, that gap is homework. Custody has to catch up, or the position is bigger than the setup can safely hold.

### Placing the couple

- **Allocation:** 60% investable / 24% net worth. Optimization-to-Sovereign territory.
- **Custody:** $150k hardware + $25k exchange. Hardware-wallet level. Appropriate for Optimization.
- **Conviction:** high. Bitcoin is a main growth driver.

Tier: Sovereign, working on it. Homework: move the $25k off the exchange to close the gap.

### No position should surprise you

If a 70% drop in your largest position would meaningfully break your plan, the position is too big for your comfort, regardless of what the percentage says. Match the size to what you can actually hold through.

### Check 1: the stress test

The stress test finds out whether the allocation you picked is one you could actually live through.

Running it now moves the decision to a day when you're calm.

#### A 70 to 80% drawdown is the normal case

A drawdown is the fall from a price peak down to the bottom that follows it, measured as a percentage of that peak.

For Bitcoin, 70 to 80% is normal, not a worst case:

- **2018:** dropped **84%**, from ~$20k to $3k.
- **2022:** dropped **77%**, from $69k to just under $16k.

You're not stress-testing a worst case. You're stress-testing the normal case.

#### The math

Your net-worth hit = the drawdown × your Bitcoin allocation.

| Allocation | 75% drawdown means |
|---|---|
| 10% | 7.5% net-worth hit |
| 25% | 19% hit |
| 50% | 37.5% hit |
| 75% | 56% hit |

You don't control the drawdown. You do control the allocation.

#### The couple's stress test

$175,000 in Bitcoin at 75% down leaves $43,750. Their $295,000 investable becomes about $164,000. $131,250 came off the screen and not one bill got smaller. The mortgage is still $2,300/mo. The car is still $360. Groceries still $1,200.

That's the pressure that makes people sell at the bottom, which is the single thing that ruins long-term growth. The stress test asks: could you sit there and not sell?

If the answer is no, you don't stop investing in Bitcoin. You adjust the allocation until you could sleep through the drawdown. You can also run this live in the app against the **Bear Market** preset (Bitcoin -60%, stocks -40%, 3-year recovery).

### Your decision

Your decision here is the largest Bitcoin position you could actually hold through a 70 to 80% drawdown without selling.

The way I think about this is pretty simple. Run the stress test on today's balance, not last year's, because the percentage doesn't move but the dollar amount does, and the dollar amount is what you'd actually feel. Answer it honestly, not aspirationally. The right answer is the one that's true at 3am with the price down 75%, not the one that sounds brave on camera. And if the honest answer is that you'd sell, then the position is too big. Most people land there the first time they run it honestly, and it's a lot better to find that out now than in the middle of the drawdown.

### Put it in Orange Plan

Strategy → Allocation. Set the Bitcoin percentage and the drift band.

### You are done when

You've run the drawdown test at today's balance, not the balance you had when you chose the number, and the answer was one you could sit through.


## 4.2 Break your accounts down by holding type, then by timeframe
*`TEACH` · 605 words · ~4 min*

**By the end of this lesson, you can:**

- Read what each of your accounts actually holds, by asset type
- Set a target mix across the six asset classes and a drift band
- Assign every account to Reserve, Bridge, or Legacy by when you'll need it
- Use the refill rule to avoid selling Bitcoin at a bad time

---

Allocation is two questions, asked in order. First: what do you actually hold, account by account, by type of asset? Second: when will you need each of those dollars? The first question gives you your mix. The second gives you your buckets. Orange Plan tracks both, and you need both, because a portfolio can have the right mix and still be sitting in the wrong timeframes.

### Step 1: what each account holds

Open **Strategy → Allocation** and scroll to **What each account holds**. Every account you own, with the holdings inside it broken out by type.

Orange Plan sorts every holding into one of six asset classes: **Bitcoin, stocks, bonds, real estate, cash, and other.**

The couple, account by account. Their investable total is $295,000.

| Account | Bitcoin | Stocks | Bonds | Cash | Total |
|---|---|---|---|---|---|
| Checking + savings | | | | $30,000 | $30,000 |
| HSA (invested) | | $10,000 | | | $10,000 |
| Taxable brokerage | | $30,000 | | | $30,000 |
| 529 | | $12,000 | $3,000 | | $15,000 |
| 401(k) + IRA | | $23,000 | $12,000 | | $35,000 |
| Bitcoin (hardware + exchange) | $175,000 | | | | $175,000 |
| **Total** | **$175,000** | **$75,000** | **$15,000** | **$30,000** | **$295,000** |

Read down the columns and you have your mix: **59% Bitcoin, 25% stocks, 5% bonds, 10% cash.**

Two things fall out of this view that you cannot see any other way. An account is not one asset. Their 401(k) is stocks *and* bonds, and the wrapper tells you nothing about what's inside it. And a single asset can be spread across accounts: their stocks live in four different places.

### Step 2: set your target mix

Same page, the **Your target mix** section. Set a target percentage for each of the six classes. They have to total 100% to save.

Underneath it is **Flag drift beyond ±**, which defaults to 5 percentage points. That's your tolerance band. Set a 60% Bitcoin target with a 5-point band and the app flags you when Bitcoin runs past 65% or falls under 55%.

The band is the useful part. Without one, every price move looks like a reason to act. With one, you only hear from the app when the mix has moved enough to matter.

The target sets the priority for **new** dollars and it's the reference for drift. It is not an instruction to sell something today.

### Step 3: sort the same accounts by when you'll need them

Now the second question. Same money, different cut.

Every dollar sits in one of three buckets. The bucket tells you what the money is for, how it should behave, and how much volatility it can carry.

| Bucket | Timeframe | Volatility | What sits here |
|---|---|---|---|
| **Reserve** | 0-3 years | Low | High-yield savings, T-bills, short Treasuries, money market. Nothing that moves. |
| **Bridge** | 4-10 years | Moderate | Taxable brokerage, HSA, 529, I-Bond ladders. Balanced mix of stocks and bonds. |
| **Legacy** | 10+ years | High | Bitcoin (in appropriate custody), retirement accounts, business equity you hold long-term. |

**Reserve** protects your ability to hold Bitcoin without being forced to sell in a bad month.

**Bridge** is accessible, flexible capital for pre-retirement needs, and the funding lane for known dated costs 4 to 10 years out.

**Legacy** is your highest-growth assets, held long enough to ride out swings. Volatility is a feature at this timeframe.

In the app it's **Funding your timeframes → Sort accounts into buckets**, and you drag each account into the bucket that matches when you'll use it. You're moving whole accounts, not individual holdings.

The couple's six accounts land like this:

| Bucket | Accounts | Amount | Share |
|---|---|---|---|
| **Reserve** | Checking + savings | $30,000 | 10% |
| **Bridge** | HSA, taxable brokerage, 529 | $55,000 | 19% |
| **Legacy** | Bitcoin, 401(k) + IRA | $210,000 | 71% |

Now look at what the two cuts tell you together. Their mix is 59% Bitcoin, which their tier supports. Their timeframes say 71% of everything they own is untouchable for a decade. Those are both fine for a couple in their forties. For a couple three years from retiring, the same mix with the same timeframes would be a problem, and only the second cut would show it.

⚠ **Anything you don't sort gets classified for you.** The app infers a bucket from the account type when you haven't set one, and it tells you which mode you're in. Inferred is a reasonable starting guess, not your decision. Sort them yourself.

⚠ **On the page, the timeframes section sits ABOVE the target mix and the account holdings.** That's a summary-first layout: the result at the top, the detail below. When you're doing the work for the first time, scroll down and start with what each account holds.

> **The refill rule is taught once, in the retirement income module (6.2).**
> It is an operating decision for when you are drawing income, not a decision
> you make while sorting accounts.

### Set a target percentage for each bucket

Each bucket has both a current balance and a target: the percentage of your investable assets it should be at.

Two things set that target, and neither is a percentage picked out of the air:

- **Reserve** you already sized in the cash-flow module, in months of spending.
- **Bridge** is the bridge you priced in the retirement module: the years between your last paycheck and your income floor.
- **Legacy** is what's left.

Both grow relative to everything else as you approach retirement. The target is what the projection uses. Change it and the plan re-solves, and from then on your job is routing new dollars toward it.

> **The three misfits moved to the walkthrough (segment 4.5, step 7b).** They
> are point-at-the-screen beats, not teaching.
>
> **The Bitcoin-in-Bridge rule is Austin's, with the number in it** (F2 resolved
> 2026-08-08, option 1):
>
> > *"Bitcoin belongs in Legacy. If you're planning to spend your Bitcoin inside
> > of 10 years, then either the bucket is wrong or the plan is wrong."*
>
> **Why ten, taught alongside the rule.** A normal Bitcoin cycle is a 70–80%
> drawdown, and recovery has historically taken years, not months. The halving
> cycle runs roughly four years. Ten years is about two full cycles: enough room
> that no single drawdown can force your hand.
>
> **The misread it must head off**, in one line: this is *not* "never sell
> Bitcoin before ten years." Module 6 has a 60-year-old selling Bitcoin to fund
> the year and that plan is correct. The difference is **whether a date can
> force the sale** — a retiree with Reserve and Bridge funded sells from Legacy
> opportunistically and can wait out a bad year; Bridge money in Bitcoin has a
> bill with a date on it and no such option. Same asset, opposite exposure.
>
> An earlier pass replaced the number with a price-dependency principle. That
> was a category-C rewrite and is reverted; the principle survives as the
> *explanation* of the rule rather than a replacement for it.

### Your decision

Your decision here is two things. A target mix across the 6 asset classes, and a target size for each of your three buckets.

Set the mix first, because it decides what you actually buy, and give it a drift band you can live with. 5 points is the default and a reasonable starting place.

Then the buckets, in order. Reserve you already sized in the reserve lesson. Bridge is driven by the years between your last paycheck and your income floor: small if you're decades away, the biggest number on the page if you're close. Legacy is whatever's left, and that's where Bitcoin belongs, because it's the only bucket with enough time to survive a drawdown.

Then compare your targets to what you actually hold. The gap between those two is your routing instruction for the next year.

### Put it in Orange Plan

Strategy → Allocation → sort every account into Reserve, Bridge or Legacy, then set each bucket's target.

### You are done when

No account is left on the app's inferred guess, and you can say where each bucket's target came from.


## 4.3 Order your contributions: which account gets funded first
*`TEACH` · 575 words · ~4 min*

> ✅ **MATH ERROR FIXED (item 22) — Option A applied 2026-07-30.** 2.1 no longer
> subtracts the 401(k) or the Roth to reach surplus, so surplus is now
> **$48,000/yr (~$4,000/mo)** = money not already committed to taxes, living, or
> debt. This routing table spends that full $4,000 once: $750 + $300 + $583 +
> $250 + $2,117. Nothing is double-counted, and the routing now genuinely decides
> the retirement contributions instead of ratifying settled ones.

> 🔴 **FILMING BLOCKER (F22) — DO NOT FILM. Order settled 2026-08-11; the
> script still needs your dictation.**
>
> The strict waterfall is gone. Austin supplied the order from his own **Week 3 ·
> Accounts + Allocation** deck (slide 8, "Build the contribution waterfall") plus
> the override condition in his own words: *"this is adjustable per situation
> based on allocation targets. If you don't have any taxable and want to retire
> early the path changes."* The doc, student-text, and visual layers below now
> teach that structure.
>
> **What still needs him:** `scripts/04-3_...md` is his spoken prose and has
> **not** been rewritten — the strict text is preserved verbatim so he can
> dictate over it against the settled structure. Layer parity between the master
> and the script is expected to disagree until he does.
>
> ⚠ Still his call, deliberately not applied: the proposed title *"Route your
> next dollar: the default order and when it changes"*. The current title says
> "which account gets funded first", which no longer quite describes a lesson
> whose answer depends on when you need the money.

**By the end of this lesson, you can:**

- Route the next dollar through the default order
- Name the conditions that change that order
- Split one dollar across two destinations when both are legitimate

---

You have a surplus. You have buckets. This lesson covers where the next dollar goes, and what changes the answer.

**This is a default order, not a command.** Most months it is the right answer and you should not overthink it. But it adjusts per situation based on your allocation targets, and the conditions that adjust it are named below.

### Three things come first

Less rungs than gates. Each either applies to you or it doesn't, and you settle it before the rest of the decision opens up.

| # | What | When it applies |
|---|---|---|
| 1 | **Reserve** | If it is still short. A reserve gap is what turns an ordinary bad month into a forced sale at the worst price. |
| 2 | **Employer match** | If you have one. 50 to 100% instant return — nothing else you do with the money gets close. If the match is 50% up to 6% of pay, contribute 6%. Not 5%. Not 3%. Six. |
| 3 | **Extra debt** | If your debt strategy says this debt wins. Module 3 already marked which debts get extra principal; this funds that decision rather than re-opening it. |

Each of these completes by being skipped deliberately. No match, no reserve gap, and nothing marked for extra principal are three finished decisions, not three unanswered ones.

### Then the order forks by time horizon

Once those are settled, the question stops being *which account is next* and becomes *when do I need this money*.

| Path | Timeframe | Where it goes | What it is for |
|---|---|---|---|
| **Legacy** | 10+ years | HSA → Roth IRA → back to the 401(k) → taxable or self-custody Bitcoin above the limits | Long-term, untouchable, inheritable |
| **Bridge** | 4–10 years | Taxable or another flexible account | Accessible, pre-retirement money |

**The default is the Legacy path.** Dollars you will not need for a decade belong in retirement accounts, in that order: the HSA if you are eligible, then the Roth IRA, then back to the 401(k) past the match, then taxable or self-custody Bitcoin for anything above the limits.

That is the typical order and it is the right starting point for most households.

### When the path changes

The default is a strong presumption, not a rule. The condition that overrides it most often:

> **You have little or nothing in taxable, and you want to retire early.**
>
> Then the Bridge path moves up. Filling every retirement account first would lock the money behind an age you are not planning to wait for, and the bridge is precisely the years between when you stop working and when those accounts open.

Two more worth naming:

- **The account cannot hold what you want to hold.** A plan with no Bitcoin option is not the better wrapper for Bitcoin dollars just because its tax treatment is better. Check what the account can hold before you route dollars to it for the tax treatment.
- **Income is uncertain.** A deduction taken this year is worth less if the year turns out worse than planned, and access is worth more.

### Deliberate splitting

One dollar can legitimately answer two needs, and splitting it is a decision, not a failure to make one. Someone building a bridge who still wants the deduction can send part of the contribution to each. Write down which part and why, so next month you are executing a decision instead of re-making it.

### Rung notes

**HSA.** Only available with a High Deductible Health Plan (HDHP). Triple-tax-advantaged — the only shelter that gets all three. Which bucket it lands in follows the job you give it: **Legacy** if you invest it and let it grow as a Roth-like medical fund, **Bridge** if you are going to spend it on medical costs before retirement.

**Roth.** Tax-free growth, tax-free withdrawal, no RMDs — no required minimum distributions, the withdrawals the government forces out of traditional accounts once you reach a certain age. Especially strong for Bitcoin-heavy plans, because Bitcoin appreciation often puts you in a higher retirement bracket than you are in now.

**Traditional.** Deduction now, taxed as ordinary income on the way out. Best where you expect a lower bracket in retirement than today.

**Income too high for a direct Roth IRA contribution.** Look up the Backdoor Roth: contribute non-deductible to a Traditional IRA, then convert. Requires you to not have other pre-tax IRA balances (pro-rata rule).

### Running the couple

$4,000/mo surplus routes:

| Step | Amount | Notes |
|---|---|---|
| Reserve | — | Already full from Module 2 |
| Employer match | $750/mo | 6% of the $150,000 salary, which captures the full 50%-up-to-6% match |
| Extra debt | — | Nothing marked for extra principal |
| HSA | $300/mo | They spend theirs on medical costs before retirement, so it sits in Bridge |
| Roth IRA | $583/mo | Maxing the year's IRA limit ($7,000 when they ran it) |
| 401(k) past the match | $250/mo | Brings the 401(k) to $12,000/yr |
| Taxable + Bitcoin | $2,117/mo | Split between Bridge and Legacy |

Every dollar has a route. Nothing is left "to figure out."

### The routing card

Post the order where you make money decisions. When a bonus or windfall arrives, walk it:

- Is the reserve whole?
- Am I capturing the full match?
- Does anything my debt strategy marked still want extra principal?
- Then: is this money for 10+ years, or do I need it before then?
- Legacy goes HSA, Roth, 401(k), then taxable or Bitcoin. Bridge goes taxable.
- Then place it inside the account according to your allocation targets.

One decision, made once, routes every future dollar — until one of the named conditions changes, which is what the annual review is for.

### Two questions about the same dollar

The order and your buckets are not competing systems. They answer two different questions about the same dollar, and both get answered every time you save.

- **The order answers: which account does it go into?** The 401(k), the HSA, the Roth, the taxable brokerage. This is the **wrapper**, and it decides how the dollar gets taxed.
- **Your buckets answer: which timeframe is it funding?** Reserve, Bridge, or Legacy. This is the **job**, and it decides what you buy once the dollar is inside.

Your Roth dollar answers both questions at once: the Roth is the wrapper, Legacy is the job. Same dollar, two labels, no conflict.

The order tells you which account, but it does not always tell you what to buy once you are inside. That is what the buckets are for. **When you have a choice inside an account, go look at which bucket is behind, and fund that one.**

**Strategy → Allocation** is the screen that shows you this. The three timeframes render with **Funded** or **Behind** badges, so you can see which one is short without doing any arithmetic. A Behind badge is the first place to *look* for this month's flexible dollars — it is not an instruction to send all of them there.

⚠ One trap worth naming: an account is not a bucket. A 401(k) sitting in a stable-value fund is a Legacy *account* holding Bridge-shaped risk. The wrapper says nothing about the job. Check both.

### Your decision

Your saved default route, the conditions that would change it, and whether this month's answer is one destination or a deliberate split.

Take free money first, because an employer match is the only guaranteed return you will ever be offered. Settle the reserve and the debts your strategy already marked. Then ask when you need the money, not which account is prettiest — that is the fork, and it is where early retirement changes the answer. Then let the buckets break the tie: once you know the account, check Allocation and put the flexible money toward whichever timeframe reads behind.

You make this decision once and it routes every future dollar without re-deciding every month. The bucket check is the only part that repeats, and it takes about 10 seconds during your monthly pass.

### Put it in Orange Plan

Cash Flow → Routing → waterfall order.

### You are done when

You have a saved default route, you know which facts override it, and you know whether this month's answer is one destination or a deliberate split. No match, no reserve gap, and no debt marked for extra principal each complete by being skipped deliberately.


## 4.4 Asset location: which account each holding belongs in
*`TEACH` · 686 words · ~5 min*

**By the end of this lesson, you can:**

- Understand the three tax wrappers and their treatment
- Place each holding in the wrapper that minimizes lifetime tax drag
- See how asset location affects the retirement withdrawal order

---

You have three types of tax wrappers. The same $1 in Bitcoin sits very differently in each. Asset location is choosing which wrapper each holding lives in, to minimize your lifetime tax drag.

### The three wrappers

| Wrapper | Tax in | Growth | Tax out | Notes |
|---|---|---|---|---|
| **Taxable brokerage** | Already-taxed dollars | Taxed annually on dividends and gains | Taxed on gains only (long-term rates if held >1yr) | Most flexible. No age limits, no penalties. |
| **Traditional (401k, IRA)** | Pre-tax (deductible) | Tax-deferred | Ordinary income | RMDs start in your seventies — the exact age depends on your birth year, and the app computes yours. 10% penalty before 59½, with a workplace-plan exception the app models. |
| **Roth (IRA, 401k)** | After-tax (post-tax) | Tax-free forever | Tax-free forever | No RMDs on Roth IRA. |

### The principle

**Put your highest-growth assets in your best tax shelter.**

Growth compounds. A high-growth asset in a taxable account pays tax on every dividend, rebalance, and realized gain, and that drag compounds too. The same asset in a Roth grows tax-free for decades.

### Where each holding belongs

**Bitcoin.** Depends. Long-term capital gains on Bitcoin are 15 to 20% federal plus state. Inside a Roth (via a Bitcoin ETF), those gains are shielded and never taxed. Best of both worlds.

The catch: Bitcoin in a Roth means you can't easily rebalance or spend it before 59½ (10% penalty). Most self-custody Bitcoin sits outside any wrapper, which makes it taxable by default.

Common pattern: some Bitcoin in retirement wrappers (ETF form) for tax shielding, the majority in self-custody (taxable) for sovereignty.

**Stocks and index funds.**

- **Broad index funds.** Roth or Traditional. High long-term growth benefits most from shielding.
- **Individual stocks.** Same. If you're actively trading, taxable is fine because you're realizing gains anyway.

**Bonds.** Traditional 401(k)/IRA (bonds throw off ordinary-income yield, so sheltering it helps), OR taxable in muni-bond form (tax-free interest for higher earners). Not usually Roth. Bonds' low growth doesn't justify tax-free-forever shielding.

**Cash / money market.** Taxable. Roth's shelter is wasted on cash's tiny growth.

**Real estate (rental).** Taxable. Depreciation and 1031 exchanges are the tax tools here, not location.

**High-yield dividend stocks.** Traditional or Roth. Dividends are taxed annually in taxable accounts.

### The couple's location

| Holding | Account | Bucket |
|---|---|---|
| $30k cash | Checking / savings | Reserve |
| $10k HSA (index funds) | HSA | Bridge |
| $30k taxable brokerage (index funds) | Taxable | Bridge |
| $15k 529 (index funds) | 529 | Bridge |
| $175k Bitcoin (hardware + exchange) | Self-custody, taxable | Legacy |
| $35k 401(k) + IRA (index funds and bonds) | Traditional 401k, IRA | Legacy |

Decent placement. If they wanted to optimize: consider a small Bitcoin ETF position inside the Roth for tax-free growth, while keeping the majority in self-custody.

### Interaction with retirement withdrawals

Where each asset sits affects the withdrawal order in retirement (Module 6):

- **Roth is generally drawn last.** Tax-free growth is the most valuable.
- **Traditional gets drawn to fill low tax brackets first.**
- **Taxable gets drawn in the middle,** for cost-basis efficiency.

If all your Bitcoin is in taxable, that dictates when it gets sold and at what rate. If some sits in Roth, you have a tax-free spendable Bitcoin pool that opens different retirement paycheck options.

### Your decision

Which holding belongs in which wrapper.

### Put it in Orange Plan

Strategy → Allocation → account holdings. Move holdings so the tax-inefficient ones sit in the sheltered accounts.

### You are done when

Every account's contents match the job of the wrapper it's in, and you can give the reason for each one.

Then watch the walkthrough below this video, where we route it in Orange Plan.


## 4.5 Walkthrough: route it in Orange Plan
*`DEMO` · 1,895 words*

> 🎥 **SCREEN SHARE — entire lesson.** Capture segment **4.6**. Beat sheet + required app state: SCREEN-SHOOT-LIST.md.

> ✅ Reflects the 2026-07-29 **Legacy** rename — the "Legacy is what the app
> calls the Forever bucket" reconciliation note is now deleted from the course.
> Proposed v2 name: **"Walkthrough: set your target mix and where new money goes"**.

**By the end of this lesson, you can:**

- Sort every account into its bucket in the app
- Save a target mix that matches your allocation tier
- Configure your 401(k) contribution row to capture the full employer match
- Use the built-in AI reviews to sanity check your setup

---

You've picked your tier, your allocation target, your buckets, and your waterfall order. This walkthrough puts it into the app so every future contribution follows the plan on its own.

Set aside about 20 minutes. Have your conviction level and target range from the earlier lessons written down. The target-mix editor won't save unless your class percentages total exactly 100%.

### Prerequisites

Two things need to be true before Step 6 will work:

- The 401(k) owner has a **salary income source** entered. Without it, Fill to match is disabled and the row explains why.
- Your surplus is routed into **Step 3** of the waterfall. The contribution rows only appear once the cash-flow pass from Module 2 has produced a surplus.

If either is missing, fix it first. Run the Module 2 cash flow walkthrough before this one.

### Step 1: Reality check on the Dashboard

Go to **Dashboard**. The hero band shows your net worth and allocation donut. Expand an account group.

Each account subline carries a tax label: **Pre-Tax**, **Roth**, **HSA**, **Tax-Free**, or **Real Estate**.

If a quantity is stale, fix it now. Three-dot menu → **Edit holding**. Everything downstream is decoration if the donut doesn't match reality.

Then flip the header **$ / ₿ toggle** in the top right. Your entire plan re-denominates in Bitcoin. Read your net worth as a Bitcoin number, sit with it for a second, and flip back. Neither denomination is wrong. Both are honest.

The toggle is app-wide, not just on the Dashboard.

### Step 2: Read what each account holds

**Strategy → Allocation → "What each account holds."** Every account as a row, with the holdings inside broken out by asset class: Bitcoin, stocks, bonds, real estate, cash, other.

Read down the columns, not across the rows. Those totals are your mix.

⚠ Two things only this view shows: an account is not one asset (a 401(k) is stocks *and* bonds), and one asset spreads across accounts.

⚠ The timeframes section renders ABOVE this one. Summary at the top, detail below. Doing the work the first time, start down here.

### Step 3: Read the three timeframes and their sufficiency badges

Go to **Strategy → Allocation → Funding your timeframes**.

This section answers the question "do I have enough in each bucket?" You don't compute it. The app renders it.

Three rows:

| Row | Purpose | Timeframe |
|---|---|---|
| **Reserve** | Cash you can spend anytime. | Now |
| **Bridge** | Early-retirement years before 59½. | Short to medium |
| **Legacy** | Long-term growth, unlocked at 59½. | Long |

Each row shows a badge: **Funded** or **Behind**.

Above the rows, a strip reads either **Assigned accounts · projected** or **Projected values**, depending on whether accounts are already sorted. If it says "Projected values," you haven't done Step 3 yet, and the sufficiency numbers are a preview.

#### How to read the badges

⚠ **A badge is information, not an order.** Behind means the app compared what you've assigned against what you told it you'd need. Only the Reserve row comes close to an instruction, because a thin reserve is what forces a sale at the worst possible moment. For the other two, Behind is a prompt to **make a decision**, not a command to drain the surplus into one row. Where a dated expense is involved, Behind should send you to the six questions in 2.3.

| Pattern | What it means | What to do |
|---|---|---|
| All three **Funded** | Your buckets carry enough for their timeframe. | Nothing urgent. Recheck at the annual review. |
| **Reserve = Behind** | Your safety net is under-funded first. | Reserve is rung 1 of the waterfall. This is the one to fix first. Route surplus to cash. |
| **Bridge = Behind**, Reserve = Funded | Your early-retirement years (before 59½) don't add up. | Decide the funding plan first; where accounts naturally sit is taxable brokerage, HSA, or 529. |
| **Legacy = Behind**, Reserve and Bridge = Funded | Long-term compounding is short. | Retirement wrappers (401(k), IRA), and any Bitcoin tier that fits your conviction. |
| **Reserve = Behind and Legacy = Funded** | Common misfit. A volatile long-term asset is doing bucket-1 work. Common with Bitcoin-heavy stacks. | Reassign accounts in Step 3, or draw cash out of Legacy to backfill Reserve. |

### Step 4: Sort your accounts into buckets

**Funding your timeframes → Assign accounts** opens a dialog titled **Sort accounts into buckets**.

The dialog instructions read *"Drag each account into the bucket that matches when you'll use it."*

Walk your list:

- Cash and savings → **Reserve**
- Taxable brokerage earmarked for a dated cost (kids' tuition, home purchase) → **Bridge**
- HSA → **Bridge** if you'll spend it on medical costs before retirement, **Legacy** if you're leaving it invested as a Roth-like medical fund. The bucket follows the job, not the account type
- 529 → **Bridge**
- Retirement accounts (401(k), IRA, Roth) → **Legacy**
- Bitcoin → **Legacy** (unless a specific chunk is reserved for a shorter timeframe)

The footer counter reads *"N accounts still unsorted"* until it flips to **"All accounts sorted."** Any unsorted account is money with no timeline job.

Close with **Done**. Watch the Funded / Behind badges update.

### Step 5: Save your target mix

**Allocation → Your target mix → Edit targets** (or **Set target mix** if none saved yet).

This is a real, saved setting. The editor heading reads **Set your target mix**, with the note *"Drag or type. Percentages must total 100% to save."*

If nothing is saved yet, four presets appear:

- **Bitcoin-forward**
- **Balanced**
- **Conservative**
- **Build your own**

Pick the range that matches the tier you defended in the allocation-tiers lesson. The line at the bottom should read **✓ 100% allocated**. If it reads *"N% allocated · adjust to 100%"*, the editor won't let you save.

Set your **drift band**: *"Flag drift beyond ± __%"*. This is how far your live mix can wander from your target before the app flags a rebalance decision. A reasonable default is ±5 percentage points, but pick a number you'd actually act on.

Commit with **Save target**.

Two things to know:

- The app records your target. It doesn't prescribe one. The number is yours to defend.
- You don't assign holdings here. The editor's own note reads: *"You don't assign accounts here. Orange Plan rolls up what they hold automatically."*

### Step 6: How the target actually gets used

Stay on **Your target mix**.

When new savings don't name a specific holding, the engine routes them by this target. Change the target, and the plan re-solves. The number you just saved is what runs the waterfall from tomorrow forward.

### Step 7: Configure the 401(k) contribution row

**Cash Flow → Routing · waterfall order → Step 3 Contributions** → click the **401(k)** row.

The collapsed row shows a mode pill: **fixed**, **fill to match**, **max**, or **leftover**.

Open the row and walk it top to bottom.

**5.1: Contribution mode** — three options:

- **Custom $/mo.** A fixed dollar amount.
- **Fill to match.** Captures the full employer match, nothing more.
- **Max.** Up to the IRS annual limit. Verify the limit for the year.

Pick **Fill to match**. The row prints *"targets $X/yr"*. The app has computed what "fill to match" actually is based on your salary and match schedule.

Beside the mode control: **annual limit $X**, which flips to *"· at annual limit"* if a higher setting would exceed the cap.

**5.2: Employer match** — two fields: **Match rate __%** and **up to __% of pay**. Enter both. The row computes *"captures $X/mo"*, the actual dollar value of the employer match you're capturing.

If Fill to match shows greyed out with *"Fill to match is off"*, the row is telling you which prerequisite is missing:

- *"Needs a salary income source on this owner to size the match."* Fix on the income screen.
- *"Needs employer match details below."* Fill in the two match fields above.

**6.3: Tax treatment** — three options: **Traditional**, **Split**, or **Roth**. Two questions to answer, in this order: can this account hold what I want to own, and which wrapper treats it best (per the asset location lesson)?

**6.4: How it invests** — three modes:

- **Current mix.** Matches what the account already holds.
- **Set mix.** A fixed target inside this account.
- **Choose holdings.** Click **+ Add holding** to add a security. A newly-added holding tags itself *"· new holding"* so you can spot it.

To add an account type that isn't listed, click **+ Add account type** at the bottom.

### Step 8: Route with AI

**Cash Flow → Routing · waterfall order → Route with AI** (top right of the section header).

This AI review reads three things: your cash flow, your contribution plan, and your debts.

Click it. Its message reads **"Route my next dollar."** It asks one clarifying question:

*"Would you like to route the amount currently available in your plan, or test a different monthly or one-time amount?"*

Answer honestly, then wait until the review lands. Read at least one thing it surfaced out loud and say whether you agree.

**When to run it:** right here, after the contribution row is configured. It's a sanity check on where the next dollar goes across reserve, debt, and contributions.

> ⚠ It reviews and explains. It does not decide.

### Step 9: Compare the change in Scenarios

**Scenarios → Custom scenario** (header button) opens a **Create scenario** dialog.

Save the current setup as a scenario so you can compare against it. Saved scenarios live under **Your scenarios**: *"Saved what-ifs you can reopen and compare."*

Then run the built-in stress test: **What if... → Bear Market**. That preset drops Bitcoin 60% and stocks 40%, then recovers over 3 years.

Ask yourself the stress-test question: could you sit through that scenario and do nothing? If not, the allocation is too high. Adjust the target in Step 4 and re-run.

### Step 10: Review portfolio (AI)

**Strategy → Allocation → Review portfolio** (page header).

This AI review reads your portfolio, the allocation target you just saved, and the funding timeframes.

Click it. It asks the question the whole module has been building to:

*"What would force you to sell investments during a deep Bitcoin drawdown?"*

Answer it out loud. If your honest answer is "a lifestyle expense I forgot to fund," go fix Reserve or Bridge before you touch the mix.

> ⚠ This review explores and explains. It won't tell you what your Bitcoin percentage should be.

### Step 11: Confirm the record

Back to **Allocation → Your target mix**.

Two facts should be true and visible:

- The target and its drift band are saved. This is your number on the record.
- The waterfall order is Reserve → Extra debt → Contributions, and it runs monthly.

### What good looks like

- Your holdings match reality. The donut doesn't lie.
- All accounts sorted. Dialog footer reads "All accounts sorted."
- Bucket badges tell the true story — but **three Funded badges is not the finish line.** The finish line is that every badge has been read and decided on. A **Behind** badge you have looked at, run through the six questions in 2.3, and made a deliberate call about is a finished item, not an outstanding one.
- Bucket-job match: nothing with Legacy-level volatility parked in Bridge money. The classic misfit is Reserve Behind while Legacy is Funded.
- Target mix at 100%, in a range you can defend, saved with a drift band you'd actually act on.
- **captures $X/mo** shows the full match captured. Anything less is free money on the table, and it's visible on the screen.
- Bear Market versus the new target: the plan survives, and your honest answer to the sit-still question is yes. If not, the allocation is too high.

### Handing it off

The waterfall runs monthly from here. Between now and the annual review, your job is to fund it. The app does the routing.

The next module covers debt strategy: which debts to attack, which to hold, and how to think about them alongside a Bitcoin position.

---

<!-- ADVANCED-GATE:START -->

## Related advanced lessons

**Your core plan is complete.** These are optional, and each one is
worth watching only when its condition is true for you. Continue only if
one of these describes your situation:

- **A4.1 The price context check: naming the emotion before a big move**
  → *Watch this before any Bitcoin-heavy move: a large buy, selling to lock in gains, or taking a Bitcoin-backed loan. Your allocation decision is complete without it.*

*Generated by `tools/build-module-gates.py` from the Gate line on each
advanced lesson. Edit the condition there, not here.*

<!-- ADVANCED-GATE:END -->
