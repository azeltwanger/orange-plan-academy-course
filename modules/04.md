...
Status: TEACHING_REWRITE_REVIEW — individual spoken replacement using the accepted Reserve reference. Integrated wording awaits Austin's voice/judgment review; app recording remains separate.
Adapted source: original Accounts + Allocation conviction paths, dollar drawdown teaching and Global Brain allocation playbook; unchanged current comparison.
App references: actual Current/Target comparison and saved choice need the approved build.
...
Preserve Foundation / Integration / Optimization / Sovereign as the current course's descriptive framework. The older deck prints indicative percentage bands; do not present them as required ranges or a progression in wealth, skill or custody. No new mandated target or suitability score is introduced.

Show hypothetical isolated stress: 50% × 70% = 35% portfolio decline; 80% × 70% = 56%; current $728,000 × 70% = $509,600 loss; $1,307,000 − $509,600 = $797,400 remaining. Label other holdings unchanged for this illustration. It is not a complete stress scenario or prediction that a given drawdown will recur.

Use the frozen 60/34/6 comparison, not a member recommendation. Its 6% cash target corresponds to $78,420 at the current scoped total, but $43,200 Reserve need is not an additional asset. Not all of the $75,000 cash/Treasury teaching category is assigned to the Reserve or necessarily interchangeable with immediately usable cash. Show that distinction when checking the dollar jobs.

### Production notes

The original deck's conviction, emotional response and drawdown concepts are retained; the current lesson's non-mandated target treatment remains. Global Brain supports conviction as an input, checking what would force a sale, family comfort, redirecting contributions and not rebalancing just to improve a chart. Do not restore the older categorical claim that a 70–80% drop is normal or guarantee a recovery. The source supports a stress illustration, not a forecast of frequency.

This is proposed edited narration. No new personal story, return assumption, source-household decision or app result is asserted. W04 chapter 2 owns actual target/preview/save behavior, same-scope comparison and supported stress displays. Saving a target does not place a trade. Final voice review and actual capture evidence remain separate.
