...
One checklist per module. This is the thing that replaces forty separate
homework blocks, and it exists so a student never has to guess whether they are
finished.

Three rules that make these work:

1. **Every line is checkable by the student, not by us.** No "understand X."
   Either a number is in the app, or a decision is written down, or it isn't.
2. **"Not applicable" is a completed line, not a skipped one.** A household with
   no debt completes the debt module by having no debt with a job. Say so
   explicitly, or people stall on rows that were never theirs.
3. **The module is done when every line is true.** Not when the videos are
   watched. Watching is not the deliverable; a plan is.

Each lesson closes with the same three beats, and the module checklist is the
sum of them:

> **YOUR DECISION** — what you decide, in your own words
> **PUT IT IN ORANGE PLAN** — where it goes, so the plan actually changes
> **YOU ARE DONE WHEN** — the checkable finish line
...
## Module 3 — Allocation, asset location, and the next dollar
...
- [ ] Your contribution waterfall is set, and you know which rung the next dollar goes to
- [ ] **Not applicable is fine:** no employer match, no HSA eligibility, and no high-interest debt each complete their rung by being skipped deliberately
...
Not when you have watched everything.
...
That last one is the difference between a plan and a document.
