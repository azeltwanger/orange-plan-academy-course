...
| forever-bucket | | `(?i)Forever[- ](bucket\|path\|lane\|channel)\|(Reserve\|Bridge\|Legacy)[ ,/]+(and )?Forever\|Forever[ ,/]+(and )?(Reserve\|Bridge\|Legacy)\|Forever.{0,3}10\+` | | `(?i)reconciliation note\|rename` | The pre-2026-07-29 bucket name. The course deliberately renamed it **Legacy** to match the app's own screen label, and `DICTATION-ORDER.md` names Reserve/Bridge/Legacy as a settle-before-you-record item precisely because renaming a bucket on camera desyncs the video from the screen. It came back on 2026-08-11 when the settled 4.3 order was transcribed from Austin's Week 3 deck, which still says "Forever path" — one deck, four layers, and nothing watching. Matching bare `Forever` was tried first and failed: it fires on ordinary English — *"a ledger you'll re-litigate forever"* — so the pattern requires the bucket-name sense (next to bucket/path/lane/channel, beside another bucket name, or against a 10+ horizon). **Written against NORMALIZED text**: `norm()` strips `**`, so an emphasis-based pattern silently never matches. The `unless` covers 4.5's note recording the rename itself |
